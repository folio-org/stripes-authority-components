...
import { getQueryStrings } from './buildQuery';
...
export const buildAdvancedSearchQuery = ({ searchOption, match, query, filters }) => {
  ...
  const templates = {
    [searchableIndexesValues.KEYWORD]: {
      ...
      [CONTAINS_ALL]: '(keyword all "%{query}" or naturalId="*%{query}*")',
    ...
    [searchableIndexesValues.PERSONAL_NAME]: () => getQueryStrings(indexData, fullTextQueryTemplate, filters),
    [searchableIndexesValues.CORPORATE_CONFERENCE_NAME]: () => getQueryStrings(indexData, fullTextQueryTemplate, filters),
    [searchableIndexesValues.GEOGRAPHIC_NAME]: () => getQueryStrings(indexData, fullTextQueryTemplate, filters),
    [searchableIndexesValues.NAME_TITLE]: () => getQueryStrings(indexData, fullTextQueryTemplate, filters),
    [searchableIndexesValues.UNIFORM_TITLE]: () => getQueryStrings(indexData, fullTextQueryTemplate, filters),
    [searchableIndexesValues.SUBJECT]: () => getQueryStrings(indexData, fullTextQueryTemplate, filters),
    [searchableIndexesValues.CHILDREN_SUBJECT_HEADING]: {
      [EXACT_PHRASE]: '((keyword=="%{query}" or naturalId="%{query}") and subjectHeadings=="b")',
      [CONTAINS_ALL]: '((keyword all "%{query}" or naturalId="%{query}") and subjectHeadings all "b")',
      [STARTS_WITH]: '((keyword all "%{query}*" or naturalId="%{query}*") and subjectHeadings all "b*")',
      [CONTAINS_ANY]: '((keyword any "%{query}" or naturalId any "*%{query}*") and subjectHeadings any "*b*")',
    },
    [searchableIndexesValues.GENRE]: () => getQueryStrings(indexData, fullTextQueryTemplate, filters),
  };
