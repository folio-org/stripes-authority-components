...
const useAuthoritiesBrowse = ({
  ...
  const authoritiesWithNull = useMemo(() => {
    ...
    // sometimes focus is wrong for Browse search so we add empty results and don't reset them
    // but focus the first row manually in useBrowseResultFocus.
    return authoritiesArray.some(Boolean) ? authoritiesArray : [];
