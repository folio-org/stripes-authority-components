...
describe('Given useAuthoritiesBrowse', () => {
  ...
  describe('when changing search index while on not first page', () => {
    it('should request first page', async () => {
      ...
      rerender({
        filters,
        searchQuery,
        searchIndex: searchableIndexesValues.CORPORATE_CONFERENCE_NAME,
        pageSize,
        precedingRecordsCount,
      }, { wrapper });
      ...
      expect(mockGet).toHaveBeenCalledTimes(3);
      expect(decodeURIWithEquals(mockGet.mock.calls[0][0]))
        .toBe('browse/authorities?limit=20&precedingRecordsCount=5&query=(headingRef>="test" or headingRef<"test") and isTitleHeadingRef==false and headingType==("Conference Name" or "Corporate Name")');
      expect(decodeURIWithEquals(mockGet.mock.calls[1][0]))
        .toBe('browse/authorities?limit=20&precedingRecordsCount=5&query=headingRef<"authority_4_0" and isTitleHeadingRef==false and headingType==("Conference Name" or "Corporate Name")');
      expect(decodeURIWithEquals(mockGet.mock.calls[2][0]))
        .toBe('browse/authorities?limit=20&precedingRecordsCount=5&query=headingRef>"authority_4_49" and isTitleHeadingRef==false and headingType==("Conference Name" or "Corporate Name")');
    });
  });
