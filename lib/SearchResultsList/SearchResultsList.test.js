...
describe('Given SearchResultsList', () => {
  ...
  describe('when a user clicks on the row', () => {
    ...
    it('should save item position to sessionStorage', () => {
      const spySetItem = jest.spyOn(Storage.prototype, 'setItem');
      const { getByRole } = renderSearchResultsList({ authorities: authorities.slice(0, 1) });

      fireEvent(getByRole('gridcell', { name: 'Twain, Mark' }), new MouseEvent('mousedown', { bubbles: true }));
