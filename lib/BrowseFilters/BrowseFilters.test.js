...
jest.mock('../MultiSelectionFacet', () => ({
  MultiSelectionFacet: ({ name, onClearFilter, onFilterChange }) => (
    <div>
      {name}
      <button type="button" onClick={() => onFilterChange(name)}>Change {name}</button>
...
const mockCloseDetailView = jest.fn();
...
describe('Given BrowseFilters', () => {
  ...
  describe('when changing a filter', () => {
    it('should close the detail view', () => {
      const { getByText } = renderBrowseFilters({ onCloseDetailView: mockCloseDetailView });

      fireEvent.click(getByText('Change headingType'));
      expect(mockCloseDetailView).toHaveBeenCalled();
    });
  });
});
