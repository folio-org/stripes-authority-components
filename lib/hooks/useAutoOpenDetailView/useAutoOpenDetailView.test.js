...
const createWrapper = (props) => {
  return function CreatedWrapper({ children }) { return <Wrapper {...props}>{children}</Wrapper>; };
};
...
describe('useAutoOpenDetailView hook', () => {
  ...
  describe('when the records is not equal to one', () => {
    it('should not open detail view', () => {
      ...
      renderHook(() => useAutoOpenDetailView(records, openDetailView), { wrapper: createWrapper(initialProps) });
  ...
  describe('when doNotShowRecord is true', () => {
    it('should not open detail view', () => {
      ...
      renderHook(() => useAutoOpenDetailView(records, openDetailView, doNotShowRecord), { wrapper: createWrapper(initialProps) });
      expect(openDetailView).not.toHaveBeenCalled();
    });
  });
