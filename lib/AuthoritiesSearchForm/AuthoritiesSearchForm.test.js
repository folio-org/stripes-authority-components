...
const mockOnShowDetailView = jest.fn();
...
describe('Given AuthoritiesSearchForm', () => {
  ...
  describe('when using Advanced Search', () => {
    ...
    describe('when making changes to rows and searching', () => {
      it('should update search textarea', () => {
        ...
        expect(getByTestId('search-textarea').value).toBe('keyword containsAll Music and keyword containsAll Painting');
      });
    });
  ...
  describe('when textarea is not empty and Reset all button is clicked', () => {
    ...
    it('should close detail view', () => {
      ...
      fireEvent.change(textarea, { target: { value: 'test search' } });
      fireEvent.click(resetAllButton);
      expect(mockOnShowDetailView).toHaveBeenCalled();
    });

    it('should focus textarea', () => {
      const { getByText, getByTestId } = renderAuthoritiesSearchForm();

      fireEvent.change(getByTestId('search-textarea'), { target: { value: 'test search' } });
      fireEvent.blur(getByTestId('search-textarea'));
      fireEvent.click(getByText('stripes-smart-components.resetAll'));
